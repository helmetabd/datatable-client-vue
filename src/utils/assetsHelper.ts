...
export function getAvatar(avatar: string | string[], url: string) {
  ...
  if (!avatar.includes('/storage/profile/')) {
    return import.meta.env.APP_STORAGE_ASSET_URL + '/uploads/avatars/' + avatar
  } else {
    return import.meta.env.APP_ASSET_URL + avatar
  }
}
...
export function getAsset(path: string | string[]) {
  if (path != null) {
    if (path[0] != '/') {
      return import.meta.env.APP_ASSET_URL + '/' + path
    } else {
      return import.meta.env.APP_ASSET_URL + path
    }
  } else {
...
export function getCsv(path: string, file_name: string) {
  if (path != null) {
    const base_url = import.meta.env.APP_ASSET_URL
